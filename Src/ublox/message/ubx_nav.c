...
void _process_eoe(uint8_t *payload, size_t payload_length) {
    ...
    LOGF("[%02d/%02d/%d %02d:%02d:%02d.%06d] Fix: %d(%d)  |  Lat: %.9f deg  | Lon: %.9f deg  | H: %.2f mm\n",
         ubx_fix.time.day, ubx_fix.time.month, ubx_fix.time.year, ubx_fix.time.hour, ubx_fix.time.minute,
         ubx_fix.time.second, ubx_fix.time.nanosecond, ubx_fix.type, !!(ubx_fix.flags & UBX_FIX_FLAG_OK),
         ubx_fix.latitude, ubx_fix.longitude, ubx_fix.height_msl);
#endif
}
